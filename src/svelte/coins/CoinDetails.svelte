<script>
  ...
  $: coin =
    $CoinStore.find((f) => f.vk === $SettingsStore.currentPage.data.vk) ||
    $SettingsStore.currentPage.data;
  $: background = hero_bg;
  $: balance =
    displayBalanceToFixed(
      BalancesStore.getBalance($currentNetwork, coin.vk),
      8
    ) || "0";
  ...
  const handleRefresh = () => {
    ...
    chrome.runtime.sendMessage({
      type: "balancesStoreUpdateOne",
      data: coin.vk,
    });
    refreshing = true;
    setTimeout(() => {
      refreshing = false;
      balance = displayBalanceToFixed(
        BalancesStore.getBalance($currentNetwork, coin.vk),
        8
      );
    }, 2000);
...
<div id="coin-details" class="flex-column text-primary">
  <div class="hero-rec flex-column" style="background-image: url({hero_bg});">
    <div class="flex-row">
      <div class="flex-column wallet-details">
        ...
        <div class="text-overlay text-body1">
  ...
  <div class="flex-row flex-align-center account-address text-body1">
    ...
    {#if addressLookupURL}
      <PopoutIcon
        width="20px"
        url={`${addressLookupURL}/addresses/${coin.vk}`}
      />
    {/if}
  </div>
</div>
