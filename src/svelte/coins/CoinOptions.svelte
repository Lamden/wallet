<script>
  ...
  const { close, setPage, setSelectedCoin } = getContext(
    "coinmodify_functions"
  );
  ...
  let options = [
    ...
    },
    {
      id: "modify-edit-btn",
      name: "Edit",
      desc: "Account Nickname",
      iconComponent: EditIcon,
      color: "primary",
      click: () => showEdit(),
    },
    {
      id: "modify-delete-btn",
      name: "Delete",
      desc: "Account from Wallet",
  ...
  $: balance = !selectedWallet
    ? "0"
    : displayBalance(
        BalancesStore.getBalance($currentNetwork, selectedWallet.vk)
      ) || "0";
  ...
  const coinList = () => {
    return $CoinStore.map((c) => {
      return {
        value: c,
        name: `${c.nickname} - ${c.vk.substring(0, 50 - c.nickname.length)}...`,
