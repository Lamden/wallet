<script>
  ...
  setContext("editor_functions", {
    checkContractExists: async (contractName, options) =>
      await getContract(contractName, options),
    addContractTab: async (contractName, contractCode) =>
      await addFileToStore(contractName, contractCode),
  });
...
{#if editorIsLoaded}
  <div id="monaco_window" class="flex-column">
    ...
    <div class="editor-row" bind:clientWidth={parentWidth}>
      <MonacoWindow
