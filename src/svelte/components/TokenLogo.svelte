<script>
    export let tokenMeta;
    export let width = '24px';
    export let margin = '0 10px';

    const genericIcon_base64_svg = "";
    
    let height = width;

    $: id = tokenMeta ? 'token-logo-' + tokenMeta.tokenSymbol : ""
    $: svgLogo = tokenMeta ? tokenMeta.logo_base64_svg || undefined : undefined;
    $: pngLogo = tokenMeta ? tokenMeta.logo_base64_png || undefined : undefined;
    $: urlB64Logo = tokenMeta ? tokenMeta.logo_base64_url || undefined : undefined;
    $: urlLogo = tokenMeta ? tokenMeta.logo_url || undefined : undefined;

    $: placeholderLogo = !svgLogo && !pngLogo && !urlLogo && !urlB64Logo ? genericIcon_base64_svg : undefined;
</script>

<style>
    img{
        border-radius: 99px;
    }
</style>

{#if svgLogo || pngLogo}
    {#if svgLogo}   
        <img id={`${id}-svg`} style={`margin: ${margin};`} {width} {height} src="{`data:image/svg+xml;base64,${svgLogo}`}" alt="token logo"/>
    {/if}

    {#if pngLogo}   
        <img id={`${id}-png`} style={`margin: ${margin};`} {width} {height} src="{`data:image/png;base64,${pngLogo}`}" alt="token logo"/>
    {/if}
{:else}
    {#if urlB64Logo}   
        <img id={`${id}-urlB64`} style={`margin: ${margin};`} {width} {height} src="{urlB64Logo}" alt="token logo"/>
    {/if}

    {#if urlLogo && !urlB64Logo}   
        <img id={`${id}-url`} style={`margin: ${margin};`} {width} {height} src="{urlLogo}" alt="token logo"/>
    {/if}
{/if}

{#if placeholderLogo}   
    <img id={`${id}-placeholder`} style={`margin: ${margin};`} {width} {height} src="{`data:image/svg+xml;base64,${placeholderLogo}`}" alt="token logo"/>
{/if}



