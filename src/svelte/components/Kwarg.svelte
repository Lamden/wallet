<script>
  ...
  //Components
  import KwargForAny from "./KwargForAny.svelte";
...
{#if argumentInfo.type === "Any"}
  <KwargForAny
    {argumentInfo}
    {index}
    {bgStyle}
    on:argChanged={() => dispatch("argChanged")}
  />
{/if}
...
{#if InputBoxTypes.includes(argumentInfo.type)}
  <InputBox
    ...
    margin="0 0 1rem 0"
    label={`${argumentInfo.name.toUpperCase()} (${
      longFormTypes[argumentInfo.type]
    })`}
